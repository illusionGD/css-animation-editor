---
name: CSS动画可视化编辑器
overview: 构建一个功能完整的CSS动画可视化编辑器，包含画布编辑、属性面板、时间轴、预设库和主题设置等核心模块，使用Vue3+TypeScript+Naive UI技术栈。
todos:
  - id: init-project
    content: 初始化Vite + Vue3 + TypeScript项目，配置ESLint + Prettier，安装Naive UI
    status: completed
  - id: setup-ai-maintenance
    content: 创建.cursorrules和memory-bank文件，建立AI维护系统
    status: completed
  - id: layout-system
    content: 实现可调整布局系统，创建顶部Banner、左右侧边栏和底部面板框架
    status: completed
    dependencies:
      - init-project
  - id: canvas-module
    content: 实现画布模块：网格、标尺、元素渲染、拖拽、选择、动画预览
    status: completed
    dependencies:
      - layout-system
  - id: property-panel
    content: 实现右侧属性面板：属性列表、编辑输入、与画布实时联动
    status: completed
    dependencies:
      - canvas-module
  - id: timeline-module
    content: 实现时间轴模块：时间轴UI、关键帧编辑、分通道显示、贝塞尔曲线编辑器、自动K帧
    status: completed
    dependencies:
      - canvas-module
  - id: preset-system
    content: 实现预设系统：Schema定义、预设列表、导入/导出功能、默认预设库
    status: completed
    dependencies:
      - layout-system
  - id: export-features
    content: 实现导出功能：CSS导出、CSS+HTML导出、JSON导出/导入
    status: completed
    dependencies:
      - timeline-module
      - property-panel
  - id: optimization
    content: 性能优化、用户体验优化、错误处理、文档完善
    status: completed
    dependencies:
      - export-features
---
